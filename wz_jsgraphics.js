...
function _mkLin2D(x1, y1, x2, y2) {
  ...
  if (dx >= dy) {
    if (dx > 0 && s - 3 > 0) {
      var _s =
        (s * dx * Math.sqrt(1 + (dy * dy) / (dx * dx)) - dx - (s >> 1) * dy) /
        dx;
    ...
    if (s - 3 > 0) {
      var _s =
        (s * dy * Math.sqrt(1 + (dx * dx) / (dy * dy)) - (s >> 1) * dx - dy) /
        dy;
      _s = (!(s - 4) ? Math.ceil(_s) : Math.round(_s)) + 1;
    } else var _s = s;
  ...
  if (jg_ok) {
    if (cnv) {
      if (typeof cnv == "string")
        this.cont = document.all
          ? this.wnd.document.all[cnv] || null
